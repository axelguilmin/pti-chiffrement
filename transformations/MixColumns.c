...
/* Si l'on représente un block dans une matrice 4,4
 * MixColumns effectue cette multiplication pour chaque colone de la matrice
...
void MixColumns( byte* state )
{
	#define X1(x) x
	#define X2(x) xtime[x]
	#define X3(x) xtime[x] ^ x

	// xtime[x] = ( x << 1 ) ^ 0x1b, => (x & 0x80 ? 0x1b : 0) ^ (x + x)
	...
	// Copie les elements un a un
	for( i=0; i<16; i++ )
		temp[i] = *( state + i ) ;
	for( i=0; i<16; i=i+4 )
	{
		*( state + i     ) = X2(temp[i]) ^ X3(temp[i+1]) ^ X1(temp[i+2]) ^ X1(temp[i+3]) ;
		*( state + i + 1 ) = X1(temp[i]) ^ X2(temp[i+1]) ^ X3(temp[i+2]) ^ X1(temp[i+3]) ;
		*( state + i + 2 ) = X1(temp[i]) ^ X1(temp[i+1]) ^ X2(temp[i+2]) ^ X3(temp[i+3]) ;
		*( state + i + 3 ) = X3(temp[i]) ^ X1(temp[i+1]) ^ X1(temp[i+2]) ^ X2(temp[i+3]) ;
	}
}

void InvMixColumns( byte* state )
{
	#define XE(x) xtimeE[x]
	#define XB(x) xtimeB[x]
	#define XD(x) xtimeD[x]
	#define X9(x) xtime9[x]

	// Tables contenant les résultats des multiplications
	...
	for( i=0; i<16; i=i+4 )
	{
		*( state + i     ) = XE(temp[i]) ^ XB(temp[i+1]) ^ XD(temp[i+2]) ^ X9(temp[i+3]) ;
		*( state + i + 1 ) = X9(temp[i]) ^ XE(temp[i+1]) ^ XB(temp[i+2]) ^ XD(temp[i+3]) ;
		*( state + i + 2 ) = XD(temp[i]) ^ X9(temp[i+1]) ^ XE(temp[i+2]) ^ XB(temp[i+3]) ;
		*( state + i + 3 ) = XB(temp[i]) ^ XD(temp[i+1]) ^ X9(temp[i+2]) ^ XE(temp[i+3]) ;
	}
}
